from app.models.security_event import STATUS_NEW, STATUS_RESOLVED, SEVERITY_LOW, SEVERITY_HIGH
...
def test_keyset_walk_has_no_dupes_or_skips(
    ...
    while True:
        ...
        if cursor:
            params["after"] = cursor
        response = client.get("/api/events", query_string=params, headers=auth_header(customer_analyst))
        assert response.status_code == 200
        body = response.get_json()
...
def test_list_filters_by_status(
    ...
    response = client.get("/api/events", query_string={"status": STATUS_RESOLVED}, headers=auth_header(customer_analyst))
...
def test_list_filters_by_severity(
    ...
    response = client.get("/api/events", query_string={"severity": SEVERITY_HIGH}, headers=auth_header(customer_analyst))
    assert _titles(response.get_json()) == ["high-one"]


def test_list_rejects_bad_filter_value(client, auth_header, customer_analyst):
    response = client.get("/api/events", query_string={"status": "not-a-status"}, headers=auth_header(customer_analyst))
    assert response.status_code == 422


def test_list_rejects_bad_cursor(client, auth_header, customer_analyst):
    response = client.get("/api/events", query_string={"after": "not-a-cursor"}, headers=auth_header(customer_analyst))
...
def test_list_rejects_unknown_query_param(client, auth_header, customer_analyst):
    response = client.get("/api/events", query_string={"not-a-param": "0"}, headers=auth_header(customer_analyst))
    assert response.status_code == 422
